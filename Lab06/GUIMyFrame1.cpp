...
void GUIMyFrame1::m_b_prewitt_click(wxCommandEvent& event)
{
    ...
    Img_Cpy = Img_Org.Copy();
    unsigned char* pix = Img_Cpy.GetData();
    unsigned char* orgPix = Img_Org.GetData();
    ...
    for (unsigned int i = 1; i < height - 1; i++) {
        for (unsigned int j = 1; j < width - 1; j++) {
            for (unsigned int k = 0; k < 3; k++) { // k = 0(R), k = 1(G), k = 2(B)
                int val = 0;
                val += orgPix[k + (i - 1) * width * 3 + (j + 1) * 3];
                val -= orgPix[k + (i - 1) * width * 3 + (j - 1) * 3];
                val += orgPix[k + i * width * 3 + (j + 1) * 3];
                val -= orgPix[k + i * width * 3 + (j - 1) * 3];
                val += orgPix[k + (i + 1) * width * 3 + (j + 1) * 3];
                val -= orgPix[k + (i + 1) * width * 3 + (j - 1) * 3];
                pix[k + i * width * 3 + j * 3] = fabs(val) / 3.0;
            }
        }
    }
}
...
void GUIMyFrame1::Contrast(int value)
{
    ...
    for (int i = 0; i < dataSize; i++) {
        int tmp = offset_c * (pix[i] - 127.0) + 127.0;
        if (tmp > 255) {
            pix[i] = 255;
        }
        else if (tmp < 0) {
